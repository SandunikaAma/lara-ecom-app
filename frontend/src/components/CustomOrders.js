...
const CustomOrders = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (editingId) {
      await axios.put(`http://localhost:3007/api/orders/${editingId}`, formData);
      setEditingId(null);
    } else {
      await axios.post("http://localhost:3007/api/orders", formData);
    }
  ...
  const handleDelete = async (id) => {
    await axios.delete(`http://localhost:3007/api/orders/${id}`);
    fetchOrders();
  };
